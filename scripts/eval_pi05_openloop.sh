#!/usr/bin/env bash
#SBATCH --job-name=eval_pi05_openloop
...
# Pi0.5 LoRA checkpoint and compare its predicted EE-space actions against the
# human-teleoperated ground-truth trajectory. No real robot / sim required.
#
# Submit:
#   sbatch scripts/eval_pi05_openloop.sh
# Override episodes/checkpoint:
#   sbatch --export=ALL,EPISODES="0 36 71",CHECKPOINT=outputs/train/.../checkpoints/last/pretrained_model scripts/eval_pi05_openloop.sh
...
CHECKPOINT="${CHECKPOINT:-$ROOT_DIR/outputs/train/pi05_so101_bowl_placement/checkpoints/010000/pretrained_model}"
EPISODES="${EPISODES:-0 36 71}"
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT_DIR/outputs/eval/pi05_openloop}"
